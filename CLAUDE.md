...
**考勤查詢部門範圍**（`getQueryScopeIds()`，bot.js）：
- 經理/董事長：全部人員（null = 不篩選）
- 主任：採樣 + 分析兩部門
- **副主任：僅自己部門**（`emp.department`，如採樣副主任只看採樣）
- 副理/簽核人員/can_approve：自己指定簽核（approver_id/2/3）的員工
- 套用於「查詢當天考勤」與「查詢當月考勤」（Rich Menu 8 格）
...
- **原因清洗**（`cleanReasonText()` bot.js / `cleanReason()` admin.js）：若使用者把「請輸入…原因：」的提示訊息（含時間文字）一併貼回，儲存與顯示時都只保留提示之後的實際內容（含已污染歷史資料的顯示）
