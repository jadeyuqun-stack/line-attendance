...
async function doQuery(emp, client, replyToken, _prefix) {
  ...
  if (lateRecords.length > 0) {
    ...
    for (var lr = 0; lr < lateRecords.length; lr++) {
      ...
      lines.push((lr2.covered ? '  ' : '  ⚠️未請假 ') + lr2.date + ' ' + lr2.time + ' 晚 ' + lr2.lateMin + ' 分' + (lr2.covered ? '' : ' 尚未請假'));
    }
  } else {
    lines.push('\n⚠️ 遲到：無');
  }
...
async function queryTodayAttendance(emp, client, replyToken) {
  ...
  if (lateList.length > 0) {
    ...
    for (var k = 0; k < lateList.length; k++) {
      ...
      lines.push((le.covered ? '  ' : '  ⚠️未請假 ') + (e3 ? e3.name + '（' + e3.employee_no + '）' : '員工#' + le.employee_id) + ' ' + timeStr + ' 遲到 ' + le.late_min + ' 分' + (le.covered ? '' : ' 尚未請假'));
    }
  }
  ...
  if (lateKeys.length > 0) {
    ...
    for (var k = 0; k < lateKeys.length; k++) {
      ...
      for (var r = 0; r < info.records.length; r++) {
        ...
        lines.push((rec.covered ? '      ' : '      ⚠️未請假 ') + rec.date + ' ' + rec.time + '（晚 ' + rec.lateMin + ' 分）' + (rec.covered ? '' : ' 尚未請假'));
      }
    }
